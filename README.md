...
Optionally, you can provide `mixWithSceneLightingColor` between 0 and 1. If provided, this will determine how the tint color is mixed with the scene lighting system color (see [here](https://github.com/DenchiSoft/VTubeStudio/wiki/Display-Light-Overlay) for more info on that). If it's set to 1, your value will overwrite the value set by the scene lighting completely. Setting it to 0 will make the scene lighting color overwrite your color. Anything in between will mix the two. If scene lighting is off, this has no effect. If you don't provide `mixWithSceneLightingColor`, it's set to 1 per default.
